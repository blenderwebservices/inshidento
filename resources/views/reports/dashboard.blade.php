...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold dash-title tracking-tight">Dashboard de Reportes & Métricas Regionales</h1>
            <p class="dash-sub text-sm mt-1">Análisis operativo y financiero para las 970 sucursales en las 9 Zonas Geográficas de Waldo's</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        ...
        <div class="glass-card rounded-2xl p-5 border-l-4 border-blue-500">
            <span class="text-[11px] font-bold card-lbl uppercase tracking-wider block">Monto Comprometido (OCs)</span>
            <span class="text-2xl font-black card-val block mt-2">${{ number_format($resumenFinanciero['monto_comprometido'], 2) }}</span>
            <span class="text-xs card-lbl block mt-1">En emisión / ejecución</span>
        </div>

        <div class="glass-card rounded-2xl p-5 border-l-4 border-emerald-500">
            <span class="text-[11px] font-bold card-lbl uppercase tracking-wider block">Monto Facturado Liquidadas</span>
            <span class="text-2xl font-black card-val block mt-2">${{ number_format($resumenFinanciero['monto_facturado'], 2) }}</span>
            <span class="text-xs card-lbl block mt-1">Liquidación fiscal final</span>
        </div>

        <div class="glass-card rounded-2xl p-5 border-l-4 border-rose-500">
            <span class="text-[11px] font-bold card-lbl uppercase tracking-wider block">Ruta de Emergencia Crítica</span>
            <span class="text-2xl font-black text-rose-500 dark:text-rose-400 block mt-2">{{ $resumenFinanciero['total_emergencias'] }} Tickets</span>
            <span class="text-xs text-rose-500 dark:text-rose-300 block mt-1">Atención inmediata (Bypass)</span>
        </div>

        <div class="glass-card rounded-2xl p-5 border-l-4 border-purple-500">
            <span class="text-[11px] font-bold card-lbl uppercase tracking-wider block">Cotizaciones Pendientes</span>
            <span class="text-2xl font-black text-purple-600 dark:text-purple-300 block mt-2">{{ $resumenFinanciero['cotizaciones_pendientes'] }} Tickets</span>
            <span class="text-xs text-purple-500 dark:text-purple-400 block mt-1">Previsión presupuestal</span>
        </div>
    </div>
    ...
    <div class="glass-card rounded-2xl overflow-hidden shadow-xl">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="table-head text-xs uppercase font-bold">
                    <tr>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60 font-medium">
                    @foreach($zonasReporte as $row)
                        <tr class="table-row-hover transition">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="px-2.5 py-1 rounded-full text-xs font-bold bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20">
                                    {{ $row['emergencias'] }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                    {{ $row['cerradas'] }}
                                </span>
                            </td>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="glass-card rounded-2xl p-6 shadow-xl space-y-4">
            <div>
                <h2 class="text-lg font-extrabold dash-title">Tablero del Ciclo de Vida (10 Pasos)</h2>
                <p class="text-xs dash-sub">Distribución en vivo de tickets según la etapa del flujo</p>
            </div>
        ...
        <div class="glass-card rounded-2xl p-6 shadow-xl space-y-4">
            <div>
                <h2 class="text-lg font-extrabold dash-title">Análisis por Disciplina / Categoría</h2>
                <p class="text-xs dash-sub">Volumen de solicitudes y costo acumulado por especialista</p>
            </div>

            <div class="space-y-4 pt-2">
                @foreach($categoriasReporte as $cat)
                    <div class="p-4 subcard-bg rounded-xl flex items-center justify-between">
                        <div class="space-y-1">
                            <h4 class="font-extrabold table-text-main text-base">{{ $cat['nombre'] }}</h4>
                            <div class="flex items-center space-x-3 text-xs dash-sub">
                                <span>Volumen: <strong class="table-text-main">{{ $cat['total_tickets'] }} incidencias</strong></span>
                            </div>
                        </div>
